...
import { approveGroupJoinRequest, rejectGroupJoinRequest } from '../actions'
...
export function GroupJoinRequests({ requests: initialRequests, groupId }: Props) {
  ...
  async function handleApprove(requestId: string) {
    ...
    const result = await approveGroupJoinRequest(requestId)
    if (!result?.error) {
  ...
  async function handleReject(requestId: string) {
    setLoading(requestId)
    const result = await rejectGroupJoinRequest(requestId)
    if (!result?.error) {
