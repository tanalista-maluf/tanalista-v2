...
export function InviteButton({ groupId, inviteToken: initialToken }: Props) {
  const [token, setToken] = useState(initialToken)
  const [copied, setCopied] = useState(false)
  const [regenerating, setRegenerating] = useState(false)

  const inviteUrl = `${typeof window !== 'undefined' ? window.location.origin : 'https://tanalista.app'}/grupos/entrar/${token}`
  ...
  async function handleRegenerate() {
    ...
    const res = await regenerateInviteTokenAction(groupId)
