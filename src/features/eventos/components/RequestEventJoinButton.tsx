...
import { requestEventJoinAction } from '../actions'
...
export function RequestEventJoinButton({ eventId, existingStatus }: Props) {
  ...
  async function handleRequest() {
    setStatus('loading')
    setError(null)
    const result = await requestEventJoinAction(eventId)
    if (result?.error) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        {status === 'loading' ? 'Enviando...' : 'Solicitar participação'}
      </button>
      {status === 'error' && error && (
