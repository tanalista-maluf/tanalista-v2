...
export async function requestWithdrawalAction(input: WithdrawalInput) {
  ...
  const total_debit = amount_cents // debita o valor solicitado; taxa já está embutida (user recebe net_cents)

  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Não autenticado.' }

  // Verificar saldo suficiente para cobrir o valor solicitado
  ...
  if (!profile || profile.wallet_balance < total_debit) {
  ...
  // Registrar requisição de saque para processamento via PIX
